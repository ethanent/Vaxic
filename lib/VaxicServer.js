			...
			req.on('end', () => {
				Object.defineProperty(req, 'cookie', {
					'get': () => {
						...
						if (lightcookie === null) {
							try {
								lightcookie = require('lightcookie')
							}
							catch (err) {
								throw new Error('The optional dependency lightcookie is not installed. ')
							}
						}

						return lightcookie.parse(req.headers.cookie)
					}
				})
				...
				const testNextRule = () => {
					if (this.handles[currentRule]) {
						...
						this.handles[currentRule].handler(req, res)
					}
				}
